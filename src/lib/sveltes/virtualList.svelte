<script lang="ts">
	...
	let visible: ArrayLike<{ index: number; data: any }>;
	let mounted: boolean;
	...
	async function refresh(items: string | any[], viewport_height: number, itemHeight: any) {
	...
	export async function scrollToIndex(index: number, opts: ScrollToOptions | undefined) {
		const { scrollTop } = viewport;
