...
function gibberish(node: Node, { delay = 0, speed = 1 }: any) {
	...
	return {
		...
		tick: (t: number) => {
			const i = Math.trunc(text.length * t);
			if (t < 1) {
				if (ticks == 0) node.textContent = randomString(text.slice(0, i).length);
...
function tooltip(node: HTMLElement, params: any = {}) {
	...
	if (params.instancesHolder) params.instancesHolder.push(tip);
...
function parseDiscordEmoji(message: string) {
	const emoteRegex = /(<a?:.+?:\d+?>)/;
	const animatedEmoteRegex = /(<a:.+?:(\d+?)>)/;
	const normalEmoteRegex = /(<:.+?:(\d+?)>)/;

	const discordURL = 'https://cdn.discordapp.com/emojis/';

	const splitted = message.split(emoteRegex);

	let progressed: string[] = [];

	for (const str of splitted) {
		//if emote
		if (str.match(normalEmoteRegex)) {
			const match = normalEmoteRegex.exec(str);
		...
		if (str.match(animatedEmoteRegex)) {
			const match = animatedEmoteRegex.exec(str);
